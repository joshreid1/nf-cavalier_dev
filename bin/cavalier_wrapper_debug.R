...
Options:
  ...
  --include-sv-csv-del        Flag to retain deletions at coding sequence (i.e. whole/partial exon deletions)
  --sv-chr-exclude=<f>        Chromosomes to exclude from SV callsets [default: chrM,chrY,M,Y].
"

opts <- ("UDP-hg38.SV.AH004-A.subset.vcf.gz AH004-A.ped S35048_2=S35048_2.merged.bam S35049_2=S35049_2.merged.bam S35152_1=S35152_1.merged.bam --sv  --exclude-benign-missense \
    --out AH004-A.SV \
    --family AH004-A \
    --genome hg38 \
    --gene-lists all_genes.tsv \
    --maf-dom 0.0001 \
    --maf-de-novo 0.0001 \
    --maf-rec 0.01 \
    --maf-comp-het 0.01 \
    ...
    (function(x) {
      ...
      gl_vars <-
        filter(x,
               gene %in% list_df$symbol,
               impact >= min_impact | 
                 (opts$include_sv_csv_del & SVTYPE == 'DEL' & str_detect(consequence, 'coding_sequence_variant')))
      # large event CNVs
      le_vars <-
